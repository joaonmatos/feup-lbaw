...
<div class="d-flex mb-2">
    <div class="flex-grow-1">
        ...
        <a href="/topics/{{ $topics[$i]["name"] }}"> #{{ $topics[$i]["name"] }} </a>
        @endfor
    </div>
    ...
    <div>
        ...
        <small class="text-muted mx-1">
            ...
            <span data-toggle="tooltip" title="Published on {{ strftime('%G-%m-%d', strtotime($story['published_date'])) }}">
                ...
                $diff = time()- $timestamp;
                for ($i = 0; $diff >= $length[$i] && $i < count($length) - 1; $i++) { $diff=$diff / $length[$i]; } $diff=round($diff); if ($diff==1) { echo $diff . " " . $str_time[$i] . "ago" ; } else { echo $diff . " " . $str_time[$i] . "s ago" ; } } @endphp </span> </small> </div> </div> <div id="comments">
